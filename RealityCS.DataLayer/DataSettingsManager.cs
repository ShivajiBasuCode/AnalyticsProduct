...
namespace RealityCS.DataLayer
{
    ...
    public partial class DataSettingsManager
    {
        ...
        public static DataSettings LoadSettings(string filePath = null, bool reloadSettings = false, IRealitycsFileProvider fileProvider = null)
        {
            if (!reloadSettings && Singleton<DataSettings>.Instance != null)
                return Singleton<DataSettings>.Instance;

            fileProvider ??= RealitycsCommonHelper.DefaultFileProvider;
            filePath ??= fileProvider.MapPath(RealitycsDataSettingsDefaults.FilePath);

            //check whether file exists
            if (!fileProvider.FileExists(filePath))
            {
                //if not, try to parse the file that was used in previous nopCommerce versions
                filePath = fileProvider.MapPath(RealitycsDataSettingsDefaults.ObsoleteFilePath);
                if (!fileProvider.FileExists(filePath))
        ...
        /// <param name="fileProvider">File provider</param>
        public static void SaveSettings(DataSettings settings, IRealitycsFileProvider fileProvider = null)
        {
            Singleton<DataSettings>.Instance = settings ?? throw new ArgumentNullException(nameof(settings));

            fileProvider ??= RealitycsCommonHelper.DefaultFileProvider;
            var filePath = fileProvider.MapPath(RealitycsDataSettingsDefaults.FilePath);
