...
{
    public partial class RealitycsEnumerationContext : RealitycsBaseContext
    {
        public RealitycsEnumerationContext()
        {
        }

        public RealitycsEnumerationContext(DbContextOptions<RealitycsEnumerationContext> options)
            : base(options)
        {
        }
        ...
        {
            modelBuilder.HasDefaultSchema("enum");
            var typeConfigurations = Assembly.GetExecutingAssembly().GetTypes().Where(type =>
                  (type.BaseType?.IsGenericType ?? false) &&
                  (type.BaseType.GetGenericTypeDefinition() == typeof(RealitycsEnumerationEntityTypeConfiguration<>)));

            foreach(var type in typeConfigurations)
            {
                var config = (IRealitycsEnumerationMappingConfiguration)Activator.CreateInstance(type);
