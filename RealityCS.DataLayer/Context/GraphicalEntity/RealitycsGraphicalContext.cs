...
{
    public partial class RealitycsGraphicalContext : RealitycsBaseContext
    {
        public RealitycsGraphicalContext()
        {
        }

        public RealitycsGraphicalContext(DbContextOptions<RealitycsGraphicalContext> options)
            : base(options)
        {
        }
      ...
      //  public virtual DbSet<tbl_Search_Parameters> tbl_Search_Parameters { get; set; }
        ...
        {
            modelBuilder.HasDefaultSchema("graphical");
            var typeConfigurations = Assembly.GetExecutingAssembly().GetTypes().Where(type =>
                  (type.BaseType?.IsGenericType ?? false) &&
                  (type.BaseType.GetGenericTypeDefinition() == typeof(RealitycsGraphicalEntityTypeConfiguration<>) ||
                  (type.BaseType.GetGenericTypeDefinition() == typeof(RealitycsGraphicalQueryTypeConfiguration<>))));

            foreach(var type in typeConfigurations)
            {
                var config = (IRealitycsGraphicalMappingConfiguration)Activator.CreateInstance(type);
