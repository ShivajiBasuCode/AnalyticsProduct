...
namespace RealityCS.ServiceCollectionExtensions.Middlewares
{
    ...
    public class InstallUrlMiddleware
    {
        ...
        public async Task Invoke(HttpContext context, IWebHelper webHelper)
        {
            ...
            {
                var installUrl = $"{webHelper.GetStoreLocation()}{RealitycsInstallationDefaults.InstallPath}";
                if (!webHelper.GetCurrentRequestUrl(false).StartsWith(installUrl, StringComparison.InvariantCultureIgnoreCase))
                {
                    //redirect
